...
const LANGUAGES: Language[] = [
  ...
  { code: 'ar', name: 'العربية', flag: '🇩🇿', dir: 'rtl' }, // Arabic is Right-to-Left
  { code: 'es', name: 'Español', flag: '🇪🇸', dir: 'ltr' },
  { code: 'sw', name: 'Kiswahili', flag: '🇹🇿', dir: 'ltr' },
];

export const LanguageSelector = () => {
  const { i18n } = useTranslation(); // Hook to control language
  const [isOpen, setIsOpen] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  // Get current language object (default to English if not found)
  ...
  const handleChangeLanguage = (lang: Language) => {
    i18n.changeLanguage(lang.code); // 1. Tell i18next to switch
    document.dir = lang.dir;        // 2. Flip the HTML document direction (LTR/RTL)
    setIsOpen(false);
  };
