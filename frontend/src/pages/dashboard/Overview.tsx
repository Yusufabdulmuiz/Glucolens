...
export default function DashboardOverview() {
  ...
  if (isLoading) {
    return (
      <AppLayout>
         <LoadingScreen message="Syncing Glucose Data..." className="h-[80vh]" />
      </AppLayout>
    );
  }
    ...
    <AppLayout>
      <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight">Health Overview</h1>
            <p className="text-gray-500 mt-1 text-sm md:text-base">
              {new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
            </p>
          </div>
          <Button onClick={() => navigate('/assessment')} className="w-full md:w-auto">
            Start New Assessment <ArrowRight className="ml-2 h-4 w-4" />
          </Button>
        </div>
        
        {/* Top Metrics Grid - RESPONSIVE: grid-cols-1 on mobile */}
        ...
        {/* Main Content Split - RESPONSIVE: Flex col on mobile */}
        <div className="flex flex-col lg:grid lg:grid-cols-7 gap-6">
          
          {/* Left Column: Chart - Full width on mobile, 4/7 on desktop */}
          ...
          {/* Right Column: Recent Activity - Full width on mobile, 3/7 on desktop */}
